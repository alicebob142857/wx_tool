...
第二个页面为 [`/writing/`](https://alicebob142857.github.io/wx_tool/writing/)。它使用独立的公众号列表，每天 06:00 检查新推文，只收录同时包含完整申论/时评范文与明确解析、点评的文章。

- 范文正文和老师点评拆成独立数据库字段，推广内容、二维码和点赞提示不进入范文。
- 按政治、经济、社会、文化、生态、科技六个大方向归类，并使用受控的细分方向；列表不显示分类标签，只在筛选器中使用。
- 范文目录默认只显示标题，点击标题后才展开完整范文和老师点评。
- 可按标题、主题、关键词、范文正文和点评全文检索，也可多选细分方向、按公众号或“只看收藏”筛选。
- 主题使用六个大方向、每个方向四个细分类的封闭字典，共 24 类；模型只能归入已有类别，不能临时创建新标签。
- 每次采集后会把全量范文同步为 GitHub Pages 静态快照。中国大陆无法连接 `workers.dev` 时，阅读和检索仍可使用。
- 收藏优先保存在当前浏览器；能连接 Worker 时同时同步到 D1。
- 页面底部可粘贴一篇公众号文章链接，或输入公众号完整名称；确认候选后即可添加、暂停、恢复或删除范文来源。
- 初始来源为“隔壁班学习园地”，示例文章会在首次有效授权后的采集中入库。

范文页与招聘页共用同一次微信公众平台扫码授权，但公众号列表和业务数据完全分开。
...
GitHub Actions (每天 22:00 UTC / 北京时间次日 06:00)
  ...
  ├─ 微信文章页：抓正文与图片
  ├─ Tesseract：中文 OCR
  ├─ DeepSeek v4：逐岗位分析，以及范文/点评分类和主题提取
  ├─ Cloudflare D1：持久化岗位数据、范文、点评和两类收藏
  └─ GitHub Pages：发布招聘页、范文页与授权二维码

Cloudflare Worker
  ├─ KV：保存最多 4 天的微信会话和 10 分钟的公众号搜索候选
  ├─ D1：岗位数据库、个性化结果与自定义要求（Worker 首次运行时幂等初始化新表）
  ├─ D1：招聘公众号与范文公众号两套独立列表及启用/暂停/删除状态
...
npm run check
npm run collect
npm run collect:writing
npm run site:serve
```

`npm run site:serve` 会在 `http://localhost:4173` 启动本地招聘页，范文页地址为 `http://localhost:4173/writing/`。范文阅读与检索优先使用 `site/writing/data/entries.json`，无需连接 Worker；公众号管理等写操作仍使用线上服务。
