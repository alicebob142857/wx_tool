...
- 可按标题、主题、关键词、范文正文和点评全文检索，也可按公众号或“只看收藏”筛选。
- 收藏保存在 D1，同一收藏状态可在不同浏览器读取。
...
`npm run site:serve` 会在 `http://localhost:4173` 启动本地招聘页，范文页地址为 `http://localhost:4173/writing/`。赞踩、两类收藏夹和公众号设置都会连接线上 Worker 并写入同一个 D1；页面顶部会显示测试提示。
