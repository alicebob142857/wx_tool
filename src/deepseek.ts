...
function feedbackPreferenceAssessment(raw: any) {
  return {
    active: Boolean(raw?.active),
    score: clampTen(raw?.score),
...
function parsePosition(raw: any, articleUrl: string, index: number): ReturnType<typeof rankPosition> | null {
  ...
  const position: RawPosition = {
    ...
    feedbackPreference: feedbackPreferenceAssessment(raw?.feedback_preference),
...
export function heuristicAnalyzeArticle(title: string, text: string, articleUrl: string): ArticleAnalysis {
  ...
  const raw: RawPosition = {
    ...
    feedbackPreference: { active: false, score: 5, reasons: [], concerns: [] },
    accessibility: 2,
    evidence: [],
...
export async function analyzeArticle(
  ...
  const feedbackPreference = userProfile.considerFeedback ? userProfile.feedbackPreference : undefined;
  const feedbackPreferenceText = feedbackPreference?.softPreferences?.length
    ? `${feedbackPreference.summary}\n${feedbackPreference.softPreferences.map(item =>
      `- ${item.direction === "prefer" ? "偏好" : "谨慎"}${item.dimension}：${item.preference}（强度 ${item.strength}/3，支持 ${item.support} 条）`,
    ).join("\n")}\n提醒：${feedbackPreference.caution}`
    : "未启用或反馈样本不足";
  const input = `求职者画像：${userProfile.school}，${userProfile.education}，${userProfile.major}专业，${userProfile.freshGraduate ? "具有应届毕业生身份" : "不限定应届身份"}。\n自定义重要要求：${customRequirement || "无"}\n是否考虑赞踩生成的偏好：${userProfile.considerFeedback ? "是" : "否"}\n赞踩生成的保守偏好：${feedbackPreferenceText}\n\n文章标题：${title}\n文章链接：${articleUrl}\n\n网页正文：\n${articleText.slice(0, 24_000)}\n\n图片 OCR：\n${ocrText.slice(0, 30_000)}`;
...
6. 如果启用了“赞踩生成的保守偏好”，只把它当作弱排序信号：逐岗位给出 0-10 的契合分，5 表示中性。它绝不能改变优质岗位硬条件，也不能仅凭一两次反馈把岗位排除；证据不充分时必须接近 5。
